...
or
[strategy.position_size](https://www.tradingview.com/pine-script-reference/v6/#var_strategy%7Bdot%7Dposition_size).
...
[Placeholders](#placeholders) section for a list.
...
[Repainting](/pine-script-docs/concepts/repainting/) section may
...
](#background)[- Which type of alert is best?
](#which-type-of-alert-is-best)[- Script alerts
](#script-alerts)[- `alert()` function events
](#alert-function-events)[- Using all `alert()` calls
](#using-all-alert-calls)[- Using selective `alert()` calls
](#using-selective-alert-calls)[- In strategies
](#in-strategies)[- Order fill events
](#order-fill-events)[- `alertcondition()` events
](#alertcondition-events)[- Using one condition
](#using-one-condition)[- Using compound conditions
](#using-compound-conditions)[- Placeholders
](#placeholders)[- Avoiding repainting with alerts
](#avoiding-repainting-with-alerts) [      ](#top)           -  [Pine Q&A chat](https://www.tradingview.com/chat/#BfmVowG1TZkKO235)  
