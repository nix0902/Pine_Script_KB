...
**URL:** https://www.tradingview.com/pine-script-docs/concepts/libraries/
...
[function](/pine-script-docs/language/user-defined-functions/),
...
[compiler annotations](/pine-script-docs/language/script-structure/#compiler-annotations) are optional but we highly recommend you use them. These
...
requires a “simple string” argument for its `symbol` parameter when a script does not allow [dynamic requests](/pine-script-docs/concepts/other-timeframes-and-data/#dynamic-requests). If we wrote a
...
[enum types](/pine-script-docs/language/type-system/#enum-types), allowing other scripts to import sets of predefined, named
...
collection of the
[enum type](/pine-script-docs/language/type-system/#enum-types) can take on:
...
[enum’s](/pine-script-docs/language/enums/) members, or when the
...
![image](/pine-script-docs/_astro/Libraries-CreatingALibrary-PublishWindow.DrHv_-QJ_Z2mzOSu.webp)
...
[compiler annotations](/pine-script-docs/language/script-structure/#compiler-annotations) we used in our library. We will publish the library
...
![image](/pine-script-docs/_astro/Libraries-UsingALibrary-1.PrDno7wM_ZsSrb9.webp)
...
####       Inputs  
 ...
 ](/pine-script-docs/concepts/inputs)  [ Next
...
[- Introduction
](#introduction)[- Creating a library
](#creating-a-library)[- Library functions
](#library-functions)[- Qualified type control
](#qualified-type-control)[- User-defined types and objects
](#user-defined-types-and-objects)[- Enum types
](#enum-types)[- Publishing a library
](#publishing-a-library)[- House Rules
](#house-rules)[- Using a library
](#using-a-library) [      ](#top)           -  [Pine Q&A chat](https://www.tradingview.com/chat/#BfmVowG1TZkKO235)  
