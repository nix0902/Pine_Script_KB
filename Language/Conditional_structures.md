...
structure used for its side effects has the following syntax:
    ...
    <local_block>{else if <expression>    <local_block>}[else    <local_block>]
where:
...
[label.set_xy()](https://www.tradingview.com/pine-script-reference/v6/#fun_label%7Bdot%7Dset_xy)
...
[declaration mode](/pine-script-docs/language/variable-declarations/#declaration-modes)
...
[name](/pine-script-docs/language/identifiers/)
...
key expression:
    ...
    {<expression> => <local_block>}    => <local_block>
...
and
[ta.crossunder()](https://www.tradingview.com/pine-script-reference/v6/#fun_ta%7Bdot%7Dcrossunder)
...
####       Operators  
 ...
 ](/pine-script-docs/language/operators)  [ Next
...
[- Introduction
](#introduction)[- `if` structure
](#if-structure)[- `if` used for its side effects
](#if-used-for-its-side-effects)[- `if` used to return a value
](#if-used-to-return-a-value)[- `switch` structure
](#switch-structure)[- `switch` with an expression
](#switch-with-an-expression)[- `switch` without an expression
](#switch-without-an-expression)[- Matching local block type requirement
](#matching-local-block-type-requirement) [      ](#top)           -  [Pine Q&A chat](https://www.tradingview.com/chat/#BfmVowG1TZkKO235)  
